...
class RlPipeline(Pipeline):
    ...
    def step(self, dry_run=False):
        ...
        if not dry_run:
        # if not dry_run:
        #     if not np.all(np.isfinite(pd_target)):
        #         logger.critical("NaN/Inf detectado en pd_target. Apagando.")
        #         self.env.shutdown()
        #         raise RuntimeError("pd_target contiene NaN o Inf")

        #     lower = np.asarray(self.env.position_limits)[:, 0]
        #     upper = np.asarray(self.env.position_limits)[:, 1]

        #     outside_limits = (pd_target < lower) | (pd_target > upper)

        #     if np.any(outside_limits):
        #         bad_indices = np.where(outside_limits)[0].tolist()

        #         logger.critical(
        #             "Objetivo fuera de límites articulares. "
        #             f"Índices: {bad_indices}"
        #         )

        #         self.env.shutdown()
        #         raise RuntimeError("Objetivo articular fuera de límites")

        #     target_delta = np.abs(pd_target - np.asarray(self.env.dof_pos))
        #     max_delta = float(np.max(target_delta))

        #     # Valor deliberadamente conservador para las primeras pruebas.
        #     if max_delta > 0.95:
        #         joint_idx = int(np.argmax(target_delta))

        #         logger.critical(
        #             "Salto excesivo del objetivo articular: "
        #             f"joint={joint_idx}, delta={max_delta:.4f} rad"
        #         )

        #         self.env.shutdown()
        #         raise RuntimeError("Salto excesivo en pd_target")
