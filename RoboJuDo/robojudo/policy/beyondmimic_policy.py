...
class BeyondMimicPolicy(Policy):
    ...
    def __init__(self, cfg_policy: BeyondMimicPolicyCfg, device):
        ...
        self.motion_fps = float(self.cfg_policy.motion_fps)
        self.playback_rate = float(self.cfg_policy.playback_rate)
        ##############
    ...
    def reset(self):
        ...
        # self.play_speed: float = 1.0 # decoment if you want to use the playback rate from the config

        # velocidad de reproduccion de la animacion, se calcula a partir de la frecuencia de control y la frecuencia de la animacion , eliminar si es necesario
        self.nominal_play_speed = (
            self.motion_fps / float(self.freq)
        ) * self.playback_rate

        self.play_speed: float = self.nominal_play_speed

        logger.info(
            "[BeyondMimicPolicy] control=%d Hz, motion=%.1f FPS, "
            "playback_rate=%.2f, frames_per_step=%.4f",
            self.freq,
            self.motion_fps,
            self.playback_rate,
            self.play_speed,
        ...
        for command in commands or []:
            match command:
                case "[MOTION_RESET]":
                    self.reset()
                case "[MOTION_FADE_IN]":
                    # self.play_speed = 1.0 # este valor estaba por default
                    self.play_speed = self.nominal_play_speed
