        ...
        if not cfg.run_fullspeed:
            ...
            if time_diff > 0:
                ...
                if not cfg.env.is_sim:
                    ...
                    if time_diff < -0.2:
                        logger.critical(
                            f"Excessive frame drop detected: {time_diff:.4f} s"
                        )

                        if hasattr(pipeline, "force_locomotion"):
                            pipeline.force_locomotion()

                            logger.warning(
                                "Switched to locomotion fallback instead of damping."
                            )

                            time.sleep(pipeline.dt)
                            continue

                        logger.error(
                            "No locomotion fallback is available. "
                            "Keeping the process active; emergency stop remains manual."
                        )
